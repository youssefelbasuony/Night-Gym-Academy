# 🎓 Course System

Project: Night Gym Academy

Parent Company: YFLB

Founder: Youssef El Basuony

Developed by: YFLB

Version: 1.0

---

# Purpose

This document defines the Course System of Night Gym Academy.

The goal is to build a professional learning system that delivers structured, scientific, and engaging fitness education.

Courses should provide a clear learning path from beginner to professional.

---

# Goals

- Deliver structured education.
- Support long-term learning.
- Track student progress.
- Prepare students for professional certifications.
- Create an enjoyable learning experience.

---

# Course Categories

- Anatomy
- Physiology
- Biomechanics
- Kinesiology
- Exercise Science
- Nutrition
- Supplements
- Training Systems
- Program Design
- Rehabilitation
- Injury Prevention
- Coaching Skills
- Business for Coaches
- Professional Certifications

---

# Course Structure

Each course contains:

Course

↓

Modules

↓

Chapters

↓

Lessons

↓

Quiz

↓

Module Exam

↓

Final Exam

↓

Certificate

---

# Every Course Must Include

- Course ID
- Course Name
- Arabic Name
- English Name
- Description
- Instructor
- Category
- Difficulty
- Estimated Duration
- Total Lessons
- Thumbnail
- Cover Image

---

# Difficulty Levels

- Beginner
- Intermediate
- Advanced
- Professional

---

# Lesson Types

- Video Lesson
- Reading Lesson
- Interactive Lesson
- PDF Lesson
- Practical Demonstration
- Case Study

---

# Learning Materials

- Videos
- Images
- Illustrations
- PDF Files
- Notes
- Downloads
- External References

---

# Learning Progress

Track:

- Current Lesson
- Completed Lessons
- Completion Percentage
- Total Study Time
- Last Activity
- XP Earned

---

# Student Features

- Continue Learning
- Bookmark Lessons
- Save Notes
- Download Resources
- Track Progress
- Course Reviews
- Ask Questions (Future)

---

# Course Navigation

- Previous Lesson
- Next Lesson
- Module Overview
- Course Overview
- Progress Bar

---

# Assessments

Each course may include:

- Lesson Quiz
- Chapter Quiz
- Module Exam
- Final Exam

Successful completion unlocks the certificate.

---

# Certificates

Each completed course may generate:

- Digital Certificate
- QR Verification
- Certificate ID
- Download PDF
- Share Certificate

---

# Search & Filters

Search by:

- Course Name
- Instructor
- Category
- Difficulty
- Language

Filter by:

- Beginner
- Intermediate
- Advanced
- Completed
- In Progress
- Free
- Premium

---

# Recommendations

Recommend courses based on:

- Previous Courses
- Exam Results
- Weak Areas
- Learning Goals
- User Interests

---

# AI Features (Future)

- AI Course Recommendations
- AI Study Plan
- AI Learning Assistant
- AI Lesson Summary
- AI Personalized Learning Path

---

# Accessibility

Support:

- Desktop
- Tablet
- Mobile
- RTL
- LTR
- Screen Readers

---

# Future Features

- Offline Learning
- Live Classes
- Workshops
- Webinars
- Group Learning
- Instructor Q&A
- Interactive Assignments
- Peer Review

---

# Scientific Standards

Every course must:

- Be evidence-based.
- Be reviewed before publication.
- Follow international fitness standards.
- Be updated regularly.

---

# Final Principle

Courses should not only teach information.

They should build real knowledge, practical skills, and professional confidence.

Night Gym Academy aims to provide a world-class learning experience for every 
student.









## Lesson System

Implemented:

- Dynamic lesson loading
- Lesson Details page
- Course Curriculum
- Resources
- Learning Outcomes
- Previous / Next Navigation
- Mark as Completed Button



# Continue Learning System

Current Status:

✔ Progress calculation completed.

✔ Dynamic button states completed.

✔ Continue Course function implemented.

⚠ Remaining Issue

Duplicate HTML exists inside the Course Card template.

This must be resolved before enabling the final Continue Learning workflow.
---

Approved By

Youssef El Basuony

Parent Company

YFLB